...
function seleccionar(vende_id, vende){
		alert(vende_id);
		opener.document.frm_mod.txt_idVende.value=vende_id;
		opener.document.frm_mod.txt_vendedor.value=vende;
		opener.document.frm_mod.lst_estado.value=2;
}
...
if ($buff == 2 || $buff == 1){
	$idCone = conectarBD();
	$SQL="Select idPerfil, Nombre , Apellido, tel, email, idUsu
		  from usuarios
		  where idPerfil = 2";
	...
	echo "<tr><th>id<th>Nombre<th>Apellido<th>Tel<th>e-mail<th></tr>";
	while ($row=mysqli_fetch_array($rs)){
		$contador = $contador + 1;
		echo "<tr>";
		echo "<td align=Center bgcolor='#eeeeee' font color='#dddddd'>".$row['idPerfil']."</td>";
...
<td align=Center bgcolor='#eeeeee' font color='#dddddd'><input type='button' value='ingresar' onClick=seleccionar(<?php echo "'$row[idUsu]'"?>,<?php echo "'$row[Nombre]'"?>)></td></tr><?php
		}
		echo "</table>";
}else{
...
<form name="frm" method="post" action="popup_vendedor.php" >
<br />
